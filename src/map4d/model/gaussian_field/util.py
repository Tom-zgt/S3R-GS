...
def initialize_gaussians(
    ...
    if w_temporal_vis:
        assert scene_timestamps is not None
        gaussians = Gaussians(
            means=xyz,
            opacities=opacities,
            scales=scales,
            ...
            scene_seqs=scene_seqs,
            scene_views=scene_views,
            views=view_nums,
            seqs=seq_nums,
            record_train_frames = torch.zeros((scene_timestamps.shape[0]),dtype=torch.bool,device=xyz.device),
        )
    else:
        gaussians = Gaussians(
            means=xyz,
    ...
    if w_temporal_vis:
        temporal_vis_tensor = torch.zeros((xyz.shape[0], 2*view_nums), dtype=torch.int, device=xyz.device)
        temporal_vis_tensor[:,1::2] = scene_timestamps.shape[0]
        point_life_tensor = torch.zeros((xyz.shape[0], 2*view_nums), dtype=torch.int, device=xyz.device)
        point_life_tensor[:,::2] = scene_timestamps.shape[0]
        gaussians.add_attribute("temporal_vis",temporal_vis_tensor,requires_grad=False)
        gaussians.add_attribute("point_life",point_life_tensor,requires_grad=False)
    return gaussians

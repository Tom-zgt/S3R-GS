...
from map4d.model.struct.gaussians import Gaussians #, Gaussians_w_tv
...
class VanillaAdaptiveDensityController:
    ...
    def __init__(
        ...
        if reset_tv_every == 0:
            self.reset_tv_every = reset_alpha_every // 3
        else:
            self.reset_tv_every = reset_tv_every
        self.reset_tv = reset_tv
    ...
    def _splitting(self, scales: Tensor, quats: Tensor, mask: Tensor) -> Gaussians:
        ...
        if self.reset_tv:
            new_other_attrs['temporal_vis'][:,::2] = 0
            new_other_attrs['temporal_vis'][:,1::2] = self.gaussians.scene_timestamps.shape[0]
            new_other_attrs['point_life'][:,1::2] = 0
            new_other_attrs['point_life'][:,::2] = self.gaussians.scene_timestamps.shape[0]
        new_gaussians = Gaussians(
            ...
            scene_timestamps=self.gaussians.scene_timestamps,
            scene_seqs=self.gaussians.scene_seqs,
            scene_views=self.gaussians.scene_views,
            views=self.gaussians.views,
            seqs=self.gaussians.seqs,
            record_train_frames=self.gaussians.record_train_frames,
        )
    ...
    def __call__(self, step: int, param_index: int, optimizers: Optimizers) -> None:
        ...
        if (
            self.reset_tv and
            (step % (self.refine_every*self.reset_tv_every)==0)
        ):
            self.gaussians.reset_tv()

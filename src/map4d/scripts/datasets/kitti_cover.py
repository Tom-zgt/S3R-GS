...
from map4d.scripts.datasets.goudingsam import segment_sky, segment_txt1
import cv2

KITTI_FRAME_RANGE = {"0001": [380, 431], "0002": [140, 224], "0006": [65, 120], "0007": [0,799], "0009": [0,800], "0020": [0,800]}

def project_to_image(points_3d, camera_matrix, image_width=1600, image_height=900, with_depth=False, is_corners=False):
    """
    Project 3D points to 2D image plane while ignoring points behind the camera and outside the image boundaries.

    :param points_3d: numpy array of shape (N, 3)
    :param camera_matrix: numpy array of shape (3, 4)
    :param image_width: int, the width of the image
    :param image_height: int, the height of the image
    :return: numpy array of shape (M, 2) where M <= N, containing only points in front of the camera and within the image frame
    """
    points_3d_hom = np.hstack((points_3d, np.ones((points_3d.shape[0], 1))))  # Convert to homogeneous coordinates
    points_2d_hom = np.dot(camera_matrix, points_3d_hom.T).T  # Project points

    # Keep only points with a positive z value (in front of the camera)
    valid_idx = points_2d_hom[:, 2] > 0
    points_2d_hom = points_2d_hom[valid_idx]

    # Avoid division by zero for z values
    if points_2d_hom.size == 0:
        if is_corners:
            return np.empty((0, 2))
        else:
            return np.empty((0, 2)), None
    
    points_2d = points_2d_hom[:, :2] / points_2d_hom[:, 2, np.newaxis]  # Convert back to 2D

    if is_corners:
        x_min = points_2d[:,0].min()
        x_max = points_2d[:,0].max()
        y_min = points_2d[:,1].min()
        y_max = points_2d[:,1].max()
        if x_min >= image_width or y_min >= image_height or x_max <= 0 or y_max <= 0:
            return []
        else:
            return [max(0.0,x_min), max(0.0,y_min) ,min(image_width,x_max),min(image_height,y_max)]
    # Filter out points that are outside the image boundaries
    in_image_idx = (points_2d[:, 0] >= 0) & (points_2d[:, 0] < image_width) & \
                   (points_2d[:, 1] >= 0) & (points_2d[:, 1] < image_height)
    new_valid_idx = valid_idx
    new_valid_idx[valid_idx] = in_image_idx
    points_2d = points_2d[in_image_idx]
    if with_depth:
        return np.concatenate((points_2d,points_2d_hom[in_image_idx][:,2:3]), axis=1),new_valid_idx
    else:
        return points_2d,new_valid_idx
    
def complete_point_cloud1(original_points, cam_intrs, cam_extrs, image_paths, image_width=1600, image_height=900, cover_mode=True):
    # depth_imgs = getdepth(image_paths)
    completed_points = np.empty((0,3))
    fit_depth_imgs = []
    lidar_depth_imgs = []
    know_depths = []
    assignments = None
    # segany_masks = segment_scene(image_paths)
    if cover_mode:
        cover_txt = ['building','house', 'tree']
        cover_masks = segment_txt1([cv2.imread(image_path) for image_path in image_paths], cover_txt, text_threshold=0.2, nms_threshold=0.8)
    cover_points_list = []
        
    index = 0
    for cam_intr, cam_extr in zip(cam_intrs,cam_extrs):
        cam_intr1 = np.hstack((cam_intr, np.zeros((3, 1))))
        cam_mat = cam_intr1.dot(cam_extr)
        project_points, valid_idx = project_to_image(original_points, cam_mat, image_width, image_height, with_depth=True)
            
        # get the idx of lidar points in the cover areas
        if cover_mode:
            cover_mask = cover_masks[index]
            cover_idxs = cover_mask[project_points[:,1].astype(np.int32),project_points[:,0].astype(np.int32)]
            
            bev_coords = original_points[valid_idx][cover_idxs,:2]
            
            grid_size = 0.5  

            
            bev_quantized = np.floor(bev_coords / grid_size).astype(int)

            
            z_coords = original_points[valid_idx][:, 2]
            
            grid_to_stats = defaultdict(lambda: {'max_z': float('-inf'), 'sum_x': 0.0, 'sum_y': 0.0, 'count': 0})
            for i in range(bev_quantized.shape[0]):
                grid = tuple(bev_quantized[i].tolist())
                x, y = bev_coords[i].tolist()
                z = z_coords[i].item()
                
                grid_to_stats[grid]['max_z'] = max(grid_to_stats[grid]['max_z'], z)
                grid_to_stats[grid]['sum_x'] += x
                grid_to_stats[grid]['sum_y'] += y
                grid_to_stats[grid]['count'] += 1
            ...
            for grid, stats in grid_to_stats.items():
                if stats['count'] < dense_thresh:
                    continue
                x_mean = stats['sum_x'] / stats['count']
                y_mean = stats['sum_y'] / stats['count']
                max_z = stats['max_z']
                
                z_vals = np.arange(max_z, max_height, z_interval)
                supplemented_points.append(np.stack([np.full_like(z_vals, x_mean), np.full_like(z_vals, y_mean), z_vals], axis=1))

            
            if len(supplemented_points) > 0:
                supplemented_points = np.concatenate(supplemented_points, axis=0)
                sup_project_points, new_idx = project_to_image(supplemented_points, cam_mat, image_width, image_height)
                sup_mask = cover_mask[sup_project_points[:,1].astype(np.int32),sup_project_points[:,0].astype(np.int32)]
                supplemented_points = supplemented_points[new_idx][sup_mask]
                cover_points_list.append(supplemented_points)
            else:
                supplemented_points = np.empty((0, 3))
                cover_points_list.append(supplemented_points)  
        
        index += 1

    all_cover_points = np.vstack(cover_points_list)
    
    return all_cover_points
   



@dataclass
class ProcessKITTI_COVER:
    data: Path = Path("data/KITTI/tracking/training")
    """path to the data"""
    sequence: Literal["0001", "0002", "0006", "0007", "0009", "0020"] = "0001"
    """sequence to use"""
    ...
    def main(self):
        ...
        for frame, line in enumerate(tqdm(lines)):
            ...
            for cam_id, camera, transformation, intrinsics in [(0, "2", P22imu, P2), (1, "3", P32imu, P3)]:
                ...
                view_exts.append(c2w)
                view_intrs.append(intrinsics[:3, :3])
                img_info = ImageInfo(
            ...
            cover_point = complete_point_cloud1(global_pc, view_intrs, view_exts, view_images, im_hws[0][0], im_hws[0][1], cover_mode=True)
            global_col = np.vstack([global_col, np.zeros((cover_point.shape[0], 3), dtype=int)])
            global_pc = np.vstack([global_pc, cover_point])
